...
| a | Listen/stop at the center frequency; graph is held during listening |
...
The receive filter edges are marked on the graph. NFM/FM/WFM/AM presets use 12.5/50/200/10 kHz widths; `b` accepts a custom 3–200 kHz width. These filters apply to listening. Spectrum still displays the captured band. `a` listens, and Space resumes the live spectrum. WFM displays RDS when redsea is installed.
