...
The file path is canonicalized. Formats include `cs8`, `cu8`, `wav`, `pcap`, `pcapng`, `nmea`, `ts`, and `hex`; a decoder should reject unsupported formats. Input files remain on disk rather than being copied into the JSON payload. Treat external input as untrusted data.
...
The host validates JSON, caps stdout at 1 MiB and enforces a process deadline. Addons run as the invoking user and inherit the environment: install only trusted code. The timeout is not a security sandbox or a resource isolation boundary. Owned process groups are killed on completion/cancellation, but descendants are not sandboxed. Network access and external binaries are the module author's responsibility.
...
See [PROTOCOLS.md](PROTOCOLS.md) for the expanded catalog, options, concurrent batch execution and backend limits. `formats` and `requires` are optional manifest metadata; formats are enforced when declared. `--options` adds an `options` JSON object to the request.
